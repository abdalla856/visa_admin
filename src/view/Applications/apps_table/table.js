...
import { getAllStudent, getAllApps, deleteApp } from "../../../actions/admin";
import { useSelector } from "react-redux";
import { Link } from "react-router-dom";
import moment from "moment";
function createData(type, createdBy, cratedAt, id) {
  return { type, createdBy, cratedAt, id };
}
...
export default function AppTable() {
  ...
  console.log(app);
  if (users.users !== undefined && app.Apps !== undefined)
    users.users.map((user, index) => {
      app.Apps.map((app) => {
        if (user.id === app.creator) {
          rows.push(
            createData(
              app.type,
              user.name,
              moment(app.new_date).format("L"),
              app.id
            )
          );
        }
            ...
            <TableBody>
              {rows.map((row) => (
                ...
                >
                  ...
                  <TableCell align="right">{row.cratedAt}</TableCell>
